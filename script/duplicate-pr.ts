...
import { createOpencode } from "@mindsparq-ai/sdk"
...
`)
  ...
  const mindsparq = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await mindsparq.client.session.create()
    const result = await mindsparq.client.session
      .prompt({
