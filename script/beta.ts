...
const model = "mindsparq/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/mindsparq")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with mindsparq...`)
  ...
  try {
    await $`mindsparq run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  mindsparq failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with mindsparq")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with mindsparq...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/mindsparq`.",
