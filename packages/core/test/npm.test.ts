...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@mindsparq/acme")).toBe("@mindsparq/acme")
    expect(Npm.sanitize("@mindsparq/acme@1.0.0")).toBe("@mindsparq/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/mindsparq/acme.git"
    const expected = win ? "acme@git+https_//github.com/mindsparq/acme.git" : spec
