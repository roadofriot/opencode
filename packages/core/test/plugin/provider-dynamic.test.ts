...
import { AISDK } from "@mindsparq-ai/core/aisdk"
...
import { DynamicProviderPlugin } from "@mindsparq-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "mindsparq-provider-dynamic-"))
