        ...
        [name]: Tool.make({
          ...
          execute: (input, context) => {
            return Effect.gen(function* () {
              ...
              const targetPath = AbsolutePath.make(target.canonical)
              const type = yield* reader.inspect(targetPath)
              if (type === "directory") return yield* reader.list(targetPath, { offset: input.offset, limit: input.limit })
              const content = yield* reader.read(targetPath, target.resource, {
                offset: input.offset,
                limit: input.limit,
              })
              if ("encoding" in content && content.encoding === "base64" && SUPPORTED_IMAGE_MIMES.has(content.mime)) {
                return yield* image
                  .normalize(target.resource, { ...content, encoding: "base64" })
                  .pipe(Effect.catchTag("Image.ResizerUnavailableError", () => Effect.succeed(content)))
              }
              if ("encoding" in content && content.encoding === "base64")
                return yield* Effect.fail(new ReadToolFileSystem.BinaryFileError(target.resource))
              return content
            }).pipe(
