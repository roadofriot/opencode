...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-mindsparq-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-mindsparq-directory"] ? decode(request.headers["x-mindsparq-directory"]) : process.cwd())
