...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "MindSparQ AI update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating MindSparQ AI", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.mindsparq/bin/mindsparq",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, mindsparqChecks: {} })
})
...
test("ignores stale background MindSparQ AI checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().mindsparqChecks).toEqual({})
})

test("ignores stale startup MindSparQ AI checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveOpencode: async () => {
      ...
      return "/home/me/.mindsparq/bin/mindsparq"
    },
  }
}
