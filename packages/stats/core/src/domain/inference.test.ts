...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve mindsparq route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "mindsparq")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "mindsparq")).toBe("openai")
    expect(statProvider("big-pickle", "", "mindsparq")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "mindsparq"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep mindsparq as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "mindsparq"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "mindsparq"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep mindsparq or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "mindsparq"), country: "US" })).toMatchObject([
