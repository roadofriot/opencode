...
Il existe deux types d'agents dans MindSparQ AI ; agents primaires et sous-agents.
...
MindSparQ AI est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
MindSparQ AI est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

MindSparQ AI est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Si aucune température n'est spécifiée, MindSparQ AI utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale MindSparQ AI et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration MindSparQ AI utilise le format `provider/model-id`. Par exemple, si vous utilisez [MindSparQ AI Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
