...
For å feilsøke problemer med MindSparQ AI, start med å sjekke loggene og lokale data den lagrer på disken.
...
MindSparQ AI lagrer øktdata og andre applikasjonsdata på disken på:
...
MindSparQ AI Desktop kjører en lokal MindSparQ AI-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `MindSparQ AI`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
MindSparQ AI kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at MindSparQ AI kan gjenoppbygge den.

1. Avslutt MindSparQ AI Desktop helt.
...
3. Start MindSparQ AI Desktop på nytt.

---

### Løs problemer med servertilkobling

MindSparQ AI Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
På Windows krever MindSparQ AI Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til MindSparQ AI.

---

### Varsler vises ikke

MindSparQ AI Desktop viser bare systemvarsler når:

- varsler er aktivert for MindSparQ AI i OS-innstillingene dine, og
...
1. Avslutt MindSparQ AI Desktop.
2. Finn og slett disse filene (de finnes i MindSparQ AI Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. MindSparQ AI installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start MindSparQ AI på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge MindSparQ AI til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
MindSparQ AI vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
