...
To debug issues with MindSparQ AI, start by checking the logs and local data it stores on disk.
...
To uninstall the MindSparQ AI CLI and remove its related files, run:
...
To remove MindSparQ AI Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

MindSparQ AI Desktop runs a local MindSparQ AI server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `MindSparQ AI` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
MindSparQ AI can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so MindSparQ AI can rebuild it.

1. Quit MindSparQ AI Desktop completely.
...
3. Restart MindSparQ AI Desktop.

---

### Fix server connection issues

MindSparQ AI Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, MindSparQ AI Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with MindSparQ AI's features.

---

### Notifications not showing

MindSparQ AI Desktop only shows system notifications when:

- notifications are enabled for MindSparQ AI in your OS settings, and
...
1. Quit MindSparQ AI Desktop.
2. Find and delete these files (they live in the MindSparQ AI Desktop app data directory):
...
If you're experiencing issues with MindSparQ AI:
