...
MindSparQ AI Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, MindSparQ AI'daki diğer sağlayıcılar gibi çalışır. MindSparQ AI Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve MindSparQ AI'u kullanmak için bunu kullanmanıza gerek yoktur.
...
MindSparQ AI Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

MindSparQ AI Go, MindSparQ AI'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>MindSparQ AI Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `MindSparQ AI Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye MindSparQ AI Go'ya abone olabilir.
:::
...
MindSparQ AI Go aşağıdaki limitleri içerir:
...
MindSparQ AI yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `opencode-go/kimi-k2.7-code` kullanmalısınız.
...
MindSparQ AI Go'yu şu amaçlarla oluşturduk:
...
4. MindSparQ AI ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
