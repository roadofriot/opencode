---
title: Eklentiler
description: MindSparQ AI'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek MindSparQ AI'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya MindSparQ AI'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
MindSparQ AI bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir MindSparQ AI SDK istemcisi.
...
MindSparQ AI'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
MindSparQ AI masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
MindSparQ AI'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca MindSparQ AI'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, MindSparQ AI'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra MindSparQ AI'a da hazır olacaktır.
