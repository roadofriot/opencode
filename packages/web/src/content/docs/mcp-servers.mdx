...
You can add external tools to MindSparQ AI using the _Model Context Protocol_, or MCP. MindSparQ AI supports both local and remote servers.
...
You can define MCP servers in your [MindSparQ AI Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
MindSparQ AI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MindSparQ AI will:
...
If the server requires authentication, MindSparQ AI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MindSparQ AI will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in MindSparQ AI, alongside built-in tools. So you can manage them through the MindSparQ AI config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MindSparQ AI to your Sentry account.
