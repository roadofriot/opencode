---
title: Konfiguracja
description: Ustawienia MindSparQ AI w pliku JSON.
---

Możesz dostosować MindSparQ AI za pomocą pliku konfiguracyjnego JSON.

---

## Format

MindSparQ AI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację MindSparQ AI w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Kiedy MindSparQ AI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer MindSparQ AI.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie MindSparQ AI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, MindSparQ AI powróci do `"build"` z ostrzeżeniem.
...
MindSparQ AI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy MindSparQ AI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie MindSparQ AI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają MindSparQ AI o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie MindSparQ AI tylko do określonych dostawców.
