---
title: CLI
description: MindSparQ AI CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The MindSparQ AI CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with MindSparQ AI programmatically.
...
Start the MindSparQ AI terminal user interface.
...
The MindSparQ AI CLI also has the following commands.

---

### agent

Manage agents for MindSparQ AI.
...
Attach a terminal to an already running MindSparQ AI backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote MindSparQ AI backend. For example:
...
MindSparQ AI is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When MindSparQ AI starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in MindSparQ AI.
...
Start a headless MindSparQ AI server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
List all MindSparQ AI sessions.
...
Delete an MindSparQ AI session.
...
Show token usage and cost statistics for your MindSparQ AI sessions.
...
Import session data from a JSON file or MindSparQ AI share URL.

```bash
opencode import <file>
```

You can import from a local file or an MindSparQ AI share URL.
...
Start a headless MindSparQ AI server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access MindSparQ AI through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run MindSparQ AI.
...
Uninstall MindSparQ AI and remove all related files.
...
MindSparQ AI can be configured using environment variables.
