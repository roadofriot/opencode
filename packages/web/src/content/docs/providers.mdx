---
title: Providers
description: Using any LLM provider in MindSparQ AI.
---

import config from "../../../config.mjs"
export const console = config.console

MindSparQ AI uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your MindSparQ AI config.
...
You can customize the providers through the `provider` section in your MindSparQ AI
config.
...
MindSparQ AI Zen is a list of models provided by the MindSparQ AI team that have been
tested and verified to work well with MindSparQ AI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with MindSparQ AI Zen.
:::

1. Run the `/connect` command in the TUI, select `MindSparQ AI Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in MindSparQ AI and is completely optional to use.

---

## MindSparQ AI Go

MindSparQ AI Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the MindSparQ AI team that have been
tested and verified to work well with MindSparQ AI.

1. Run the `/connect` command in the TUI, select `MindSparQ AI Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with MindSparQ AI:
...
MindSparQ AI. Anthropic explicitly prohibits this.

Previous versions of MindSparQ AI came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in MindSparQ AI with zero setup:
...
MindSparQ AI supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; MindSparQ AI uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize MindSparQ AI. Sign in and approve.

   :::note
   MindSparQ AI requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, MindSparQ AI surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — MindSparQ AI forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in MindSparQ AI is experimental. Features, configuration, and
behavior may change in future releases.
:::

MindSparQ AI integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `MindSparQ AI`, Scopes: `api`
...
MindSparQ AI uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock MindSparQ AI
...
When a `duo-workflow-*` model is selected, MindSparQ AI will:
...
4. Route tool execution requests through MindSparQ AI's permission-gated tool system
...
To use Google Vertex AI with MindSparQ AI:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each MindSparQ AI conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each MindSparQ AI conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for MindSparQ AI. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with MindSparQ AI:
...
3. Copy the API key for use in MindSparQ AI.
...
6. **Important**: Before using cloud models in MindSparQ AI, you must pull the model information locally:
...
MindSparQ AI Zen is a list of tested and verified models provided by the MindSparQ AI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>MindSparQ AI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **MindSparQ AI Zen**.

   ```txt
   /connect
   ```

3. Enter your MindSparQ AI API key.
...
5. Complete sign-in in the browser that opens. MindSparQ AI captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. MindSparQ AI opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
MindSparQ AI refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when MindSparQ AI is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running MindSparQ AI — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. MindSparQ AI prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. MindSparQ AI polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
The `limit` fields allow MindSparQ AI to understand how much context you have left. Standard providers pull these from models.dev automatically.
