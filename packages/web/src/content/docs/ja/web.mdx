---
title: ウェブ
description: ブラウザで MindSparQ AI を使用する。
---

MindSparQ AI はブラウザで Web アプリケーションとして実行でき、ターミナルを必要とせずに同じ強力な AI コーディングエクスペリエンスを提供します。

![MindSparQ AI Web - 新しいセッション](../../../assets/web/web-homepage-new-session.png)
...
これにより、利用可能なランダムなポートを使用して `127.0.0.1` でローカルサーバーが起動され、デフォルトのブラウザで MindSparQ AI が自動的に開きます。
...
デフォルトでは、MindSparQ AI は使用可能なポートを選択します。ポートを指定できます。
...
デフォルトでは、サーバーは `127.0.0.1` (localhost のみ) にバインドされます。ネットワーク上で MindSparQ AI にアクセスできるようにするには:

```bash
opencode web --hostname 0.0.0.0
```

`0.0.0.0` を使用すると、MindSparQ AI はローカルアドレスとネットワークアドレスの両方を表示します。
...
開始すると、Web インターフェイスから MindSparQ AI セッションにアクセスできるようになります。

### セッション

ホームページからセッションを表示および管理します。アクティブなセッションを確認したり、新しいセッションを開始したりできます。

![MindSparQ AI Web - アクティブなセッション](../../../assets/web/web-homepage-active-session.png)

### サーバーステータス

「サーバーを表示」をクリックすると、接続されているサーバーとそのステータスが表示されます。

![MindSparQ AI Web - サーバー](../../../assets/web/web-homepage-see-servers.png)
