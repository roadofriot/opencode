...
Možete dodati vanjske alate u MindSparQ AI koristeći _Model Context Protocol_, ili MCP. MindSparQ AI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MindSparQ AI Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
MindSparQ AI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MindSparQ AI će:
...
Ako server zahtijeva autentifikaciju, MindSparQ AI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MindSparQ AI sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MindSparQ AI, zajedno s ugrađenim alatima. Možete njima upravljati kroz MindSparQ AI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MindSparQ AI sa vašim Sentry nalogom.
