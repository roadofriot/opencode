...
Per diagnosticare problemi con MindSparQ AI, inizia controllando i log e i dati locali che salva su disco.
...
MindSparQ AI Desktop esegue in background un server locale di MindSparQ AI (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `MindSparQ AI` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
MindSparQ AI puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che MindSparQ AI possa ricostruirla.

1. Chiudi completamente MindSparQ AI Desktop.
...
3. Riavvia MindSparQ AI Desktop.

---

### Risolvi problemi di connessione al server

MindSparQ AI Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, MindSparQ AI Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di MindSparQ AI.

---

### Notifiche non visualizzate

MindSparQ AI Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per MindSparQ AI nelle impostazioni del sistema operativo, e
...
1. Chiudi MindSparQ AI Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app MindSparQ AI Desktop):
...
Se riscontri problemi con MindSparQ AI:
