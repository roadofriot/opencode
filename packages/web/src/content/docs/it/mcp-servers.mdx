...
Puoi aggiungere strumenti esterni a MindSparQ AI usando il _Model Context Protocol_, o MCP. MindSparQ AI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [MindSparQ AI Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
MindSparQ AI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, MindSparQ AI:
...
Se il server richiede autenticazione, MindSparQ AI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, MindSparQ AI memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in MindSparQ AI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione MindSparQ AI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare MindSparQ AI al tuo account Sentry.
