---
title: CLI
description: Opzioni e comandi della CLI di MindSparQ AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di MindSparQ AI, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con MindSparQ AI in modo programmatico.
...
Avvia l'interfaccia testuale di MindSparQ AI (interfaccia utente da terminale).
...
La CLI di MindSparQ AI include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per MindSparQ AI.
...
Collega un terminale a un backend server di MindSparQ AI già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend MindSparQ AI remoto. Per esempio:
...
MindSparQ AI si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando MindSparQ AI si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in MindSparQ AI.
...
Avvia un server MindSparQ AI headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Gestisci le sessioni MindSparQ AI.
...
Elenca tutte le sessioni MindSparQ AI.
...
Mostra statistiche di utilizzo token e costo per le sessioni MindSparQ AI.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione MindSparQ AI.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione MindSparQ AI.
...
Avvia un server MindSparQ AI headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a MindSparQ AI tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla MindSparQ AI e rimuove tutti i file correlati.
...
MindSparQ AI può essere configurato tramite variabili d'ambiente.
