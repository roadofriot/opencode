---
title: Provider
description: Usa qualsiasi provider LLM in MindSparQ AI.
---

import config from "../../../../config.mjs"
export const console = config.console

MindSparQ AI usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di MindSparQ AI.
...
configurazione di MindSparQ AI.
...
MindSparQ AI Zen e una lista di modelli forniti dal team MindSparQ AI, testati e
verificati per funzionare bene con MindSparQ AI. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da MindSparQ AI Zen.
:::
...
Funziona come qualsiasi altro provider in MindSparQ AI ed e totalmente opzionale.
...
Per usare Amazon Bedrock con MindSparQ AI:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in MindSparQ AI non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `MindSparQ AI`, Scopes: `api`
...
MindSparQ AI usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare MindSparQ AI
...
Per usare Google Vertex AI con MindSparQ AI:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di MindSparQ AI come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di MindSparQ AI elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per MindSparQ AI. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con MindSparQ AI:
...
3. Copia la chiave API da usare in MindSparQ AI.
...
6. **Importante**: Prima di usare modelli cloud in MindSparQ AI, devi scaricare le informazioni del modello localmente:
...
MindSparQ AI Zen è una lista di modelli testati e verificati forniti dal team MindSparQ AI. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>MindSparQ AI Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **MindSparQ AI Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di MindSparQ AI.
...
I campi `limit` permettono a MindSparQ AI di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
