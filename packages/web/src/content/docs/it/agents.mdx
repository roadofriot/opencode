...
In MindSparQ AI esistono due tipi di agenti: agenti primari e subagenti.
...
MindSparQ AI include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
MindSparQ AI include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

MindSparQ AI include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di MindSparQ AI, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Se non specifichi una temperature, MindSparQ AI usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di MindSparQ AI sia per la config specifica del progetto.
...
L'ID modello nella configurazione MindSparQ AI usa il formato `provider/model-id`. Per esempio, se stai usando [MindSparQ AI Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
