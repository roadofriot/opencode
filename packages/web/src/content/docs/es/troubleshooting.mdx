...
Para depurar problemas con MindSparQ AI, comience verificando los registros y los datos locales que almacena en el disco.
...
MindSparQ AI Desktop ejecuta un servidor MindSparQ AI local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `MindSparQ AI` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
MindSparQ AI también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que MindSparQ AI pueda reconstruirlo.

1. Salga de MindSparQ AI Desktop por completo.
...
3. Reinicie el escritorio MindSparQ AI.

---

### Solucionar problemas de conexión del servidor

MindSparQ AI Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
En Windows, el escritorio MindSparQ AI requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de MindSparQ AI.

---

### Notificaciones que no se muestran

MindSparQ AI Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para MindSparQ AI en la configuración de su sistema operativo, y
...
1. Salga del escritorio MindSparQ AI.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio MindSparQ AI):
...
Si tiene problemas con MindSparQ AI:
