---
title: CLI
description: Opciones y comandos de la CLI de MindSparQ AI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de MindSparQ AI de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con MindSparQ AI mediante programación.
...
Inicie la interfaz de usuario del terminal MindSparQ AI.
...
La CLI de MindSparQ AI también tiene los siguientes comandos.

---

### agent

Administrar agentes para MindSparQ AI.
...
Conecte una terminal a un servidor backend MindSparQ AI que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto MindSparQ AI. Por ejemplo:
...
MindSparQ AI funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia MindSparQ AI, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en MindSparQ AI.
...
Inicie un servidor MindSparQ AI sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Administrar MindSparQ AI sesiones.
...
Enumere todas las sesiones MindSparQ AI.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones MindSparQ AI.
...
Importe datos de sesión desde un archivo JSON o una URL compartida MindSparQ AI.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida MindSparQ AI.
...
Inicie un servidor MindSparQ AI sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a MindSparQ AI a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale MindSparQ AI y elimine todos los archivos relacionados.
...
La CLI de MindSparQ AI toma las siguientes banderas globales.
...
MindSparQ AI se puede configurar mediante variables de entorno.
