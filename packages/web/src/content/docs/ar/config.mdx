---
title: الإعدادات
description: استخدام ملف إعدادات MindSparQ AI بصيغة JSON.
---

يمكنك ضبط MindSparQ AI باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم MindSparQ AI تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
ضع إعدادات MindSparQ AI العامة في `~/.config/opencode/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
عند تشغيل MindSparQ AI، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم MindSparQ AI.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات MindSparQ AI عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول MindSparQ AI استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات MindSparQ AI عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود MindSparQ AI إلى `"build"` مع تحذير.
...
سيقوم MindSparQ AI بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
[الإضافات](/docs/plugins) توسّع MindSparQ AI بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد MindSparQ AI لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
