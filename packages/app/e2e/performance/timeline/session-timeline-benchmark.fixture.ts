...
const directory = "C:/MindSparQ/TimelineStateRegression"
...
const model = { providerID: "mindsparq", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimelineBenchmark(page: Page, options: { historyTurns: number; eventBatch: number }) {
  ...
  await mockMindsparqServer(page, {
    directory,
    project: project(),
    provider: provider(),
    sessions: [session()],
...
function provider() {
  return {
    ...
    connected: ["mindsparq"],
    default: { providerID: "mindsparq", modelID: "claude-opus-4-6" },
  }
}
