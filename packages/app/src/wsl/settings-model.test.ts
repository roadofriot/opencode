...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when MindSparQ AI needs attention", () => {
    ...
    ).toBe("Install MindSparQ AI")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/mindsparq",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update MindSparQ AI")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/mindsparq",
  ...
  test("probes the selected distro before entering the MindSparQ AI step", async () => {
    const calls: string[] = []
    ...
    expect(calls).toEqual(["Debian", "mindsparq"])
  })
})
