...
type WslServerStep = "wsl" | "distro" | "mindsparq"

const STEPS: WslServerStep[] = ["wsl", "distro", "mindsparq"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const mindsparqCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.mindsparqChecks[distro] ?? null
  })
  ...
  const mindsparqReady = createMemo(() => {
    const check = mindsparqCheck()
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-mindsparq" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && mindsparqReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-mindsparq"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.mindsparqChecks[distro]) {
      return { key: `probe-mindsparq:${distro}`, run: () => api.probeOpencode(distro) }
    }
    return null
  })
  ...
  const mindsparqMessage = createMemo(() => {
    ...
    if (mindsparqReady()) {
      return distro
        ? language.t("wsl.onboarding.mindsparqReadyIn", { distro })
        : language.t("wsl.onboarding.mindsparqReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.mindsparq"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : mindsparqCheck()?.matchesDesktop === false
                  ? "warning"
                  : mindsparqReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "mindsparq"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.mindsparq")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!mindsparqReady() || mindsparqCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{mindsparqMessage()}</div>
                <Show when={mindsparqCheck()?.matchesDesktop === false ? mindsparqCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "mindsparq" && allReady() && selectedDistro()}>
