...
import { Tag } from "@mindsparq-ai/ui/tag"
            ...
            <Tooltip
              ...
              value={
                <ModelTooltip
                  model={item}
                  latest={item.latest}
                  free={item.provider.id === "mindsparq" && (!item.cost || item.cost.input === 0)}
                />
              }
            >
              {node}
              ...
              >
                {(i) => (
                  <div class="w-full flex items-center gap-x-3">
                    ...
                    <span>{i.name}</span>
                    <Show when={i.id === "mindsparq"}>
                      <div class="text-14-regular text-text-weak">{language.t("dialog.provider.mindsparq.tagline")}</div>
                    </Show>
                    <Show when={i.id === "mindsparq"}>
                      <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                    </Show>
                    <Show when={i.id === "mindsparq-go"}>
                      <>
                        <div class="text-14-regular text-text-weak">
                          {language.t("dialog.provider.mindsparqGo.tagline")}
                        </div>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </>
                    </Show>
