...
export function TerminalPanel(props: { embedded?: boolean }) {
  ...
  const opened = createMemo(() => props.embedded ? layout.bottomPanel.opened() : view().terminal.opened())
  const size = createSizing()
  const height = createMemo(() => layout.terminal.height())
  const close = () => {
    if (props.embedded) {
      layout.bottomPanel.close()
    } else {
      view().terminal.close()
    }
  }
  ...
  if (props.embedded) {
    return (
      <div ref={root} class="relative w-full h-full bg-background-stronger">
        {innerContent()}
      </div>
    )
  }
