...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("mindsparq://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("mindsparq://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("mindsparq://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("mindsparq://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("mindsparq://open-project")).toBeUndefined()
    expect(parseDeepLink("mindsparq://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "mindsparq://open-project?directory=/a",
      "mindsparq://other?directory=/b",
      "mindsparq://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("mindsparq://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("mindsparq://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("mindsparq://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("mindsparq://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "mindsparq://new-session?directory=/a",
      "mindsparq://open-project?directory=/b",
      "mindsparq://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MINDSQARQ__: {
        deepLinks: ["mindsparq://open-project?directory=/a"],
      },
    } as unknown as Window & { __MINDSQARQ__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["mindsparq://open-project?directory=/a"])
