import type {
  ...
  Session,
  Todo,
} from "@mindsparq-ai/sdk/v2/client"
...
import { NormalizedProviderListResponse } from "@mindsparq-ai/ui/context"
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("mindsparq-test"))
            .slice()
