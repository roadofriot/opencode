...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("mindsparq-find")
  api.delete("mindsparq-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("mindsparq-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("mindsparq-find", new Highlight(...rest))
    return true
  }
