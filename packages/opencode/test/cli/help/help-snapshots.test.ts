...
// terminal sizes. The default mindsparq tui command is excluded —
// `mindsparq --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `mindsparq --help` prints today;
...
// top-level help on `--help` and exits 1; not a real mindsparq command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("mindsparq CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ mindsparq }) =>
      Effect.gen(function* () {
        const topLevel = yield* mindsparq.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* mindsparq.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`mindsparq ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`mindsparq ${argv.join(" ")} --help`)
        }
