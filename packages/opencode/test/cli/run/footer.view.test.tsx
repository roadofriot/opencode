...
import type { QuestionRequest } from "@mindsparq-ai/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@mindsparq-ai/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "mindsparq", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "mindsparq", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("mindsparq")
    expect(frame).toContain("GPT-5")
