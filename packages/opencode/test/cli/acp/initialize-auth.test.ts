...
describe("mindsparq acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ mindsparq }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ mindsparq }))
        ...
        expect(initialized.agentInfo?.name).toBe("MindSparQ AI")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mindsparq }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ mindsparq })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("mindsparq-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "mindsparq-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ mindsparq }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("mindsparq-login")
