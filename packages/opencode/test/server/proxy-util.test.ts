...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips mindsparq-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-mindsparq-directory": "/home/user/project",
          "x-mindsparq-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-mindsparq-directory")).toBeNull()
      expect(result.get("x-mindsparq-workspace")).toBeNull()
