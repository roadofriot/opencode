...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".mindsparq")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("hides task background parameter unless experimental background subagents are enabled", () =>
    Effect.gen(function* () {
      ...
      const task = (yield* registry.tools({
        providerID: ProviderV2.ID.mindsparq,
  ...
  it.instance("loads tools from .mindsparq/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const mindsparq = path.join(test.directory, ".mindsparq")
      const tool = path.join(mindsparq, "tool")
  ...
  it.instance("ignores non-tool exports in .mindsparq/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".mindsparq", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-mindsparq` was registering a tool
  ...
  it.instance("loads tools from .mindsparq/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const mindsparq = path.join(test.directory, ".mindsparq")
      const tools = path.join(mindsparq, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".mindsparq", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const mindsparq = path.join(test.directory, ".mindsparq")
        const customTools = path.join(mindsparq, "tools")
        const plugin = path.join(mindsparq, "node_modules", "@mindsparq-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(mindsparq, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@mindsparq-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@mindsparq-ai/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".mindsparq", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(mindsparq, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@mindsparq-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(mindsparq, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(mindsparq, "node_modules", "cowsay")
