// Subprocess test harness for the mindsparq CLI. Spawns the real binary against
...
//
// Configuration flows through mindsparq's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for mindsparq.json
...
//
// Today only `mindsparq.run` is fully wired. The shape supports adding more
// builders (`mindsparq.serve(opts)`, `mindsparq.acp(opts)`, `mindsparq.auth(...)`)
...
const mindsparqRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(mindsparqRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `mindsparq run`. New flags should
...
// `mindsparq serve` is a long-lived process — it never exits on its own.
...
// `mindsparq acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `mindsparq serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `mindsparq acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly mindsparq: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("mindsparq.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("mindsparq.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "mindsparq server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `mindsparq serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("mindsparq.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const mindsparq: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, mindsparq })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `mindsparq.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "mindsparq") {
...
// every call site. Use this for any test that needs the mindsparq CLI fixture.
//
...
// resources (e.g. `mindsparq.serve`) without an extra `Effect.scoped` wrapper —
