...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "mindsparq.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "mindsparq.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mindsparq.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mindsparq.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["mindsparq.json", "mindsparq.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "mindsparq.json"),
        path.join(project, "mindsparq.jsonc"),
      ])
    })
  })
