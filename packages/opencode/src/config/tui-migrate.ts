...
const TUI_SCHEMA_URL = "https://mindsparq.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from mindsparq.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const mindsparq = await mindsparqFiles(input)
  for (const file of mindsparq) {
...
async function mindsparqFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "mindsparq"),
    ...(await Filesystem.findUp(["mindsparq.json", "mindsparq.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "mindsparq"))
  }
