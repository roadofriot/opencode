...
import { InstallationLocal, InstallationVersion } from "@mindsparq-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@mindsparq-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@mindsparq-ai/core/v1/config/error"
...
import { Npm } from "@mindsparq-ai/core/npm"
...
export class Service extends Context.Service<Service, Interface>()("@mindsparq/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["mindsparq.jsonc", "mindsparq.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://mindsparq.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://mindsparq.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mindsparq.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mindsparq.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://mindsparq.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/mindsparq`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://mindsparq.ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("mindsparq", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".mindsparq") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["mindsparq.json", "mindsparq.jsonc"]) {
          ...
          // Auto-discovered plugins under `.mindsparq/plugin(s)` are already local files, so ConfigPlugin.load
