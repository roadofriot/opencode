...
import type { LLMClientShape } from "@mindsparq-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("mindsparq"))
    return { type: "unsupported", reason: "provider is not openai, mindsparq, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @mindsparq-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains mindsparq-owned. The native runtime only adapts
      // the @mindsparq-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
