...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall mindsparq and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall MindSparQ AI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g mindsparq-ai",
      pnpm: "pnpm uninstall -g mindsparq-ai",
      bun: "bun remove -g mindsparq-ai",
      yarn: "yarn global remove mindsparq-ai",
      brew: "brew uninstall mindsparq",
      choco: "choco uninstall mindsparq",
      scoop: "scoop uninstall mindsparq",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "mindsparq", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using MindSparQ AI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# mindsparq") || content.includes(".mindsparq/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".mindsparq/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".mindsparq/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".mindsparq"))
    ) {
      continue
    }
