import type { PermissionV1 } from "@mindsparq-ai/core/v1/permission"
// CLI entry point for `mindsparq run`.
//
// Handles three modes:
...
//      mindsparq server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@mindsparq-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run mindsparq with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running mindsparq server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'mindsparq')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://mindsparq.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
