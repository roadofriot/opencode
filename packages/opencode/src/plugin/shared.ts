...
export const DEPRECATED_PLUGIN_PACKAGES = ["mindsparq-openai-codex-auth", "mindsparq-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, mindsparqVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(mindsparqVersion) || semver.major(mindsparqVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.mindsparq
  if (typeof range !== "string") return
  if (!semver.satisfies(mindsparqVersion, range)) {
    throw new Error(`Plugin requires mindsparq ${range} but running ${mindsparqVersion}`)
  }
}
