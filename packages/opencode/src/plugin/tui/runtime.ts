...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@mindsparq-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@mindsparq-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@mindsparq-ai/tui/context/theme"
import { Global } from "@mindsparq-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@mindsparq-ai/core/util/flock"
import { Flag } from "@mindsparq-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@mindsparq-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@mindsparq-ai/core/v1/config/plugin"
import { createCommandShim } from "@mindsparq-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@mindsparq-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".mindsparq"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".mindsparq", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".mindsparq", "tui.json"),
  }
}
