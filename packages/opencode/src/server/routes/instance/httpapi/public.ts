...
import { MindsparqHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for mindsparq serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(MindsparqHttpApi)
...
export const PublicApi = MindsparqHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "mindsparq",
    version: "1.0.0",
    description: "mindsparq api",
