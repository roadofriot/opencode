...
import { createOpencode, type ToolPart } from "@mindsparq-ai/sdk"
...
console.log("🚀 Starting mindsparq server...")
const mindsparq = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await mindsparq.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new mindsparq session...")
    const { client, server } = mindsparq
    ...
    console.log("✅ Created mindsparq session:", createResult.data.id)
  ...
  console.log("📝 Sending to mindsparq:", message.text)
