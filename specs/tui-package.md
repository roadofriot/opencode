...
Move the canonical MindSparQ AI terminal application from
...
```text
packages/opencode ---\
                      > @mindsparq-ai/tui -> @mindsparq-ai/sdk
packages/cli --------/
```
...
`packages/cli`, or `@mindsparq-ai/core`.

The SDK is the TUI's MindSparQ AI boundary. Missing backend data or operations must
...
- Do not replace private `packages/opencode` imports with `@mindsparq-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@mindsparq-ai/tui`.

## Ownership Boundary

### `@mindsparq-ai/tui` Owns
...
- MindSparQ AI domain data displayed by the TUI
...
Status: Completed. The private `@mindsparq-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@mindsparq-ai/tui`.
...
- The package has no dependency on `opencode`, `@mindsparq-ai/cli`, or
  `@mindsparq-ai/core`.
...
primitives now live in `@mindsparq-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@mindsparq-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads MindSparQ AI globals or process state;
...
Replace process-global MindSparQ AI state with resolved TUI inputs.
...
`@mindsparq-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@mindsparq-ai/tui`. Configured references resolve through the new
...
adapters rather than MindSparQ AI domain implementations.

Make the SDK the only MindSparQ AI domain boundary used by the TUI.
...
- Domain-facing TUI code imports MindSparQ AI data and operations only from
  `@mindsparq-ai/sdk`.
...
`@mindsparq-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@mindsparq-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@mindsparq-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@mindsparq-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for MindSparQ AI domain state.
...
rg '@mindsparq-ai/core|packages/opencode|packages/cli' packages/tui
